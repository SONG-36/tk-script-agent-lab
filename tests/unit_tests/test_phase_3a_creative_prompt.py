...
def request_with_knowledge() -> CreativeGenerationRequest:
    ...
    return CreativeGenerationRequest(
        ...
        creative_knowledge_items=pack.items[:2],
        idea_count=2,
    )
...
def test_creative_prompt_v2_separates_business_evidence_and_guidance() -> None:
    ...
    assert context["creative_guidance"][0]["knowledge_id"] == "ck_hook_visible_micro_mess"  # type: ignore[index]
...
def test_creative_prompt_v2_does_not_allow_knowledge_as_source_usage() -> None:
    ...
    assert "ck_hook_visible_micro_mess" not in allowed_source_ids["product_fact"]
    assert "ck_hook_visible_micro_mess" not in allowed_source_ids["selling_point"]
    assert "ck_hook_visible_micro_mess" not in allowed_source_ids["reference_insight"]
